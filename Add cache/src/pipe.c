...
void pipe_stage_mem()
{
    ...
    if (op->is_mem)
    {
        if(compare_data(op->mem_addr & ~3))
            val=getData(op->mem_addr & ~3);
        else
        {
            if(delay_data())
                val=moveFromMainMemory_data(op->mem_addr & ~3);
            else
                return ;
        }
    }
...
void pipe_stage_fetch()
{
    ...
    if(compare_inst(pipe.PC))//hit
    {
            ...
            printf("hit\n");
        #endif
        op->instruction=getInstruction_inst(pipe.PC);
    }
    else    //miss
    {
        #ifdef DEBUG
        ...
        #endif
        if(delay_inst())
            op->instruction=moveFromMainMemory_inst(pipe.PC);
        else
            return ;
